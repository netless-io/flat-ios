...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _roundMargin = 10.0;
        _showSpace = 0.f;
        _containerBorderWidth = 0.0f;
        _containerBorderColor = [UIColor whiteColor];
        _shadowColor = [UIColor whiteColor];
        _containerBackgroudColor = [UIColor whiteColor];
        _containerCornerRadius = 8.0;
...
- (void)updateSubViewFrames
{
    ...
    CGRect showBounds = _showFrom.bounds;
    CGRect newBounds = CGRectMake(showBounds.origin.x - (ScreenWidth - 667)/2, showBounds.origin.y - 2, showBounds.size.width, showBounds.size.height);
    CGRect newFrame = [_showFrom convertRect:newBounds toView:window];
